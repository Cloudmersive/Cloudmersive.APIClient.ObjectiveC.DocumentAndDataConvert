...
#import "CMJSONRequestSerializer.h"
#import "CMQueryParamCollection.h"
#import "CMDefaultConfiguration.h"

NSString *const CMResponseObjectErrorKey = @"CMResponseObject";

static NSString * const kCMContentDispositionKey = @"Content-Disposition";

static NSDictionary * CM__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * CM__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = CM__headerFieldsForResponse(response);
    if(!headers[kCMContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kCMContentDispositionKey];
...
@interface CMApiClient ()

@property (nonatomic, strong, readwrite) id<CMConfiguration> configuration;
...
+ (instancetype) sharedClient {
    static CMApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[CMDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[CMDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<CMConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[CMResponseDeserializer alloc] init];
        _sanitizer = [[CMSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        CMJSONRequestSerializer * swgjsonRequestSerializer = [CMJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kCMApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        ...
        if (responseObject) {
            ...
            userInfo[CMResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = CM__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<CMSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = CMPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = CMPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, CMPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[CMQueryParamCollection class]]){
            CMQueryParamCollection * coll = (CMQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = CMPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = CMPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = CMPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<CMConfiguration> config = self.configuration;
