#import "CMConvertImageApi.h"
#import "CMQueryParamCollection.h"
#import "CMApiClient.h"
#import "CMGetImageInfoResult.h"
#import "CMMultipageImageFormatConversionResult.h"


@interface CMConvertImageApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CMConvertImageApi

NSString* kCMConvertImageApiErrorDomain = @"CMConvertImageApiErrorDomain";
NSInteger kCMConvertImageApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) convertImageGetImageInfoWithInputFile: (NSURL*) inputFile
    ...
    if (inputFile == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kCMConvertImageApiErrorDomain code:kCMConvertImageApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/convert/image/get-info"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"CMGetImageInfoResult*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CMGetImageInfoResult*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) convertImageImageSetDPIWithDpi: (NSNumber*) dpi
    ...
    if (dpi == nil) {
        NSParameterAssert(dpi);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"dpi"] };
            NSError* error = [NSError errorWithDomain:kCMConvertImageApiErrorDomain code:kCMConvertImageApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/convert/image/set-dpi/{dpi}"];
...
///  @param inputFile Input file to perform the operation on. 
///
///  @returns CMMultipageImageFormatConversionResult*
///
-(NSURLSessionTask*) convertImageMultipageImageFormatConvertWithFormat1: (NSString*) format1
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/convert/image-multipage/{format1}/to/{format2}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"CMMultipageImageFormatConversionResult*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CMMultipageImageFormatConversionResult*)data, error);
                                }
                            }];
}
